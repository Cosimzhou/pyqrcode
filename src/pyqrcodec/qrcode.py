...
@author: zhouzhichao
'''
# qrcode.QREncode("https://qr.alipay.com/tsx01576wncgn5notusc43").printOnConsole()
import random
try:
...
class QRDataEncoder(object):
    ...
    def __init__(self, version=1, mode=None, quality=0):
        self.version = version
        self.quality = quality
        self.mode    = None #'8-bit'
        self.setMode(mode)
        self.data    = None
    ...
    def encode(self, content):
        if self.mode is None:
            self.mode = QRDataEncoder.chooseMode(data)

        vq = None
        while True:
            ldata = self.assumeLength(content)
            lbyte = ldata / 8 + (1 if ldata % 8 else 0)
            vq = QRMaster.getVersionAndQualityByDataLength(lbyte, self.quality)
            vdiff = self.isFitVersion(vq[0])
            if vdiff == 0:
                self.version = vq[0]
                self.quality = vq[1]
                break
            self.shiftVersion(vdiff)

        prdata = self.encodeData(content)
        prdata = self.fillContent(prdata, QRMaster.dataCaps(*vq))    #

        pinfo = QRMaster.splitBlocks(*vq)
        self.data = self.encodeBlocksCheckCode(prdata, pinfo)
...
class QRCoder(QRMaster):
    ...
    def piecesInfo(self):
        return super(QRCoder, self).splitBlocks(self.__version, self.__quality)
    def getInfo(self):
    ...
    def __isOnAnchor(self, x, y):
        ...
        disArr = self.__anchorDistributionList(al-1)
        lx, ly = 0, 0
        x -= 4
        if x >= disArr[0]:
            x -= disArr[0]
    ...
    def __anchorDistributionList(self, anchor=None):
    ...
    def __versionAnchorDistribution(self):
        if self.__error: return
        anchor = self.anchorLevel()-1
        if anchor < 1: return
        diffarr = self.__anchorDistributionList(anchor)
        x = 6
    ...
    def setAnchor(self):
        if self.__error or not self.__matrix: return
        vad = self.__versionAnchorDistribution()
        if not vad: return
        for i in vad:
    ...
    def fillData(self, datas):
        pieceInfo = super(QRCoder, self).splitBlocks(self.__version, self.__quality)
    ...
    def extractData(self):
        ...
        for i in self.dataAreaPosition():
            x, y = i
            cdata = bool2char((self.__matrix[x][y] == BBP)^func(x,y))
            datas[igroup] += cdata
        ...
        return map(lambda x:x[:(len(x)//8)*8],datas)
    def evaluate(self):
...
def QREncode(data, level=None, mode=None):
    if mode is None:
